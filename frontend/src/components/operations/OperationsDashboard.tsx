...
export function OperationsDashboard({
  ...
  async function action(path: string, body?: unknown) {
    // PATCH handles lifecycle transitions; POST is reserved for new collection payments.
    try {
      await request(path, {
      ...
      // Refresh after a conflict because another executive may have already moved this loan forward.
      setRefresh((value) => value + 1);
    }
  }
    ...
    <AppShell user={user} onLogout={onLogout} eyebrow="Operations dashboard">
      <section className="workspace ops">
        ...
        <div className="module-strip">
          {["SALES", "SANCTION", "DISBURSEMENT", "COLLECTION"].map((item) => {
        ...
        <div className="ops-metrics">
          <MetricCard label="Applied" value={counts.APPLIED || 0} tone="blue" />
          <MetricCard
            label="Sanctioned"
          ...
          <MetricCard
            ...
            tone="purple"
          />
          <MetricCard label="Closed" value={counts.CLOSED || 0} tone="green" />
        </div>
        <div className="table-card">
          <div className="table-head">
            <div>
              ...
              <h2>
                ...
                {isAdmin ? "loans in portfolio" : "loans need attention"}
              </h2>
          ...
          {activeModule === "SALES" ? (
            ...
            <LoanList
              loans={visibleLoans}
              user={user}
              onApprove={(id) => action(`/loans/${id}/sanction`)}
              onReject={(id) => setRejectionLoanId(id)}
              onDisburse={(id) => action(`/loans/${id}/disburse`)}
              onPayment={(id, payment) =>
                action(`/loans/${id}/payments`, payment)
              }
            />
          )}
        </div>
          ...
          <div className="loan-person">
            ...
            {loan.salarySlip?.secureUrl && (
              <a
                ...
                href={loan.salarySlip.secureUrl}
                target="_blank"
                rel="noreferrer"
              >
                View salary slip ↗
